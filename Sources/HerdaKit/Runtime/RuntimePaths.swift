...
public struct RuntimePaths: Sendable {
    ...
    public func configContents(themeName: String) -> String {
        ...
        //
        // The three pane keys hand pane chrome over to native cards. They are a
        // set — each covers a different failure:
        //
        // - `pane_borders = false` gives `Borders::NONE` (herdr
        //   `src/ui/panes.rs:112`) and is also the precondition for the shrink
        //   branch at `:103`.
        // - `pane_gaps = true` then shrinks each pane with a neighbour by one
        //   cell (`shrink_for_one_cell_gap`, `:82`). That cell is where the
        //   native card gap and border go — measured 8.0pt at 13pt, the same
        //   value as `ChromeMetrics.cardGap`.
        // - `pane_scrollbars = false` is not cosmetic: left on,
        //   `stable_terminal_inner_rect` (`:34`) makes the rendered area one
        //   column narrower than the rect `pane.layout` reports, so every slice
        //   would be off by a column — and herdr's character scrollbar would sit
        //   next to the native one.
        //
        // With all three, `rect == inner_rect == PTY size == rendered area`,
        // which is what makes slicing the single grid by rect correct.
        //
        // The three modal keys silence the dialogs herdr raises on its own. Those
        // appear without a keypress, and a modal is drawn on the whole grid, so
        // once the grid is sliced by pane rect it gets cut by the card gap.
        // Modals a prefix key opens (Navigator, GlobalMenu) are left alone: the
        // user asked for those, falling back to whole-grid rendering is an
        // explainable response, and replacing them natively is a later stage.
        //
        // `prompt_new_workspace_name` already defaults to false; it is written
        // out so the three read as one set — missing one lets a class of dialog
        // back in.
        """
